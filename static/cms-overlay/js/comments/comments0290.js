...
var Comments = {
	...
	/**
	 * Toggle a comment as spam
	 */
	toggleSpam: function(id) {
		$.ajax({
			url: Core.baseUrl + '/discussion/comment/' + id + '/toggleSpam',
			type: 'POST',
			data: {
				sig: Comments.sig,
				xstoken: Cookie.read('xstoken')
			...
			dataType: 'json',
			success: function(response) {
				var url = location.pathname + "#post-" + id;
			...
			error: function(response) {
				...
				Overlay.open(responseData.errorMessage || response.statusText);
			}
		});
	...
	/**
	 * Set a comment as ham
	 */
	setHam: function(id) {
		$.ajax({
			url: Core.baseUrl + '/discussion/comment/' + id + '/ham',
			type: 'POST',
			data: {
				sig: Comments.sig,
				xstoken: Cookie.read('xstoken')
