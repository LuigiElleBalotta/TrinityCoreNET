		...
		Tooltip.factory('a[href*="/item/"], [data-item]', {
			...
			onShow: function(event) {
				...
				if (pathname.indexOf(0) != '/') {
					// workaround for browsers that may not prefix pathname with '/'
					// http://stackoverflow.com/questions/956233/javascript-pathname-ie-quirk
					pathname = '/' + pathname;
				}
				var id = pathname.split(Core.baseUrl + '/item/')[1];
			...
			onShow: function(event) {
				...
				if (typeof data != 'undefined') {
					if (type == 'scenario') {
						...
						var query = '';
						if(itemData){ query = '?itemId=' + itemData; }
						Tooltip.show(node, '/'+ type + '/'+ data +'/tooltip' + query, true);
					} else {
