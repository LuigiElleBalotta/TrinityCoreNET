...
var Lightbox = {
    ...
    setEmbed: function(embed){
        ...
        switch (embed.type) {
            case "youtube":
				var url = "https://www.youtube.com/v/" + embed.src;
                ...
                $("<object width='"+embed.width+"' height='"+embed.height+"'>" +
                    ...
                    "<embed src='"+url+"' type='application/x-shockwave-flash' allowfullscreen='true' allowScriptAccess='always' wmode='opaque' width='"+embed.width+"' height='"+embed.height+"'/>" +
                    "</object>").appendTo(Lightbox.content);
                break;
            case "netease":
                var foo = $("<object id='FPlayer' data='http://v.163.com/swf/video/NetEaseFlvPlayerV3.swf' type='application/x-shockwave-flash' width='"+embed.width+"' height='"+embed.height+"'>"+
                        "<param value='true' name='allowFullScreen'/>"+
                        "<param value='always' name='allowscriptaccess'/>"+
                        "<param value='http://v.163.com/swf/video/NetEaseFlvPlayerV3.swf' allownetworking='all' name='movie'/>"+
                        "<param name='wmode' value='opaque' />"+
                        "<param value='topicid="+embed.topicid+
                                "&amp;vid="+embed.vid+
                                "&amp;sid=V5KBLH2CT"+embed.sid+
                                "&amp;threadListPath="+
                                "&amp;coverpic="+embed.coverpic+
                                "&amp;autoplay=true"+
                            "' name='flashvars'/>"+
                    "</object>");
                    foo.appendTo(Lightbox.content);
                break;
            default:
                break;
        }
        ...
        // Update title if supplied
        if(Lightbox.config.showTitle && embed.title){
    ...
    /**
    * Load content function for common lightbox
    * @param {Array} contents - array of content objects containing title (optional), node, height, and width of the content
     *  Example:
     *    [{
     *      key:      "my-content", (arbitrary named key for finding index by name)
     *      title:    "Content Title",
     *      node:     "#content-id" (also accepts js dom node references with or without jquery wrappers),
     *      height:   "400",
     *      width:    "500"
     *    }]
    */
    loadContent : function(contents, dontStore, showIndex, advanceOnClick) {
        if (advanceOnClick == undefined) {
            Lightbox.config.advanceOnClick = true;
        } else {
            Lightbox.config.advanceOnClick = advanceOnClick;
        }
        ...
        //store data
        if(!dontStore){
            Lightbox.storeContentData(contents, "content");
        }

        //if passed a key string for a showIndex, find that indice
        if(typeof showIndex == 'string'){
            for(var i=0; i<contents.length; i++){
                if(contents[i].key == showIndex){
                    showIndex = i;
                    break;
                }
            }
        ...
        var index = showIndex || ((typeof contents === 'number') ? contents : 0);

        Lightbox.currentIndex = index;

        //set first video
        Lightbox.setContent(contents[index]);
    },

    /**
    * Load content function for common lightbox
    * @param {Object} content Content object specified in loadContent
    */
    setContent : function(content) {
        ...
        // Update title if supplied
        if(Lightbox.config.showTitle && content.title){
            Lightbox.title.html(content.title || "");
        }

        Lightbox.setFrameDimensions(content.width, content.height);
        Lightbox.show();
    ...
    setFrameDimensions: function(width, height) {
        if (width === 0 || height === 0) {
            ...
            // Get the scroll offset with fallback for IE8 & below
    ...
    close: function() {
        ...
        if(Lightbox.contentType === "content") {
            Lightbox.emptyContent();
        }
    ...
    build: function() {
        ...
        Lightbox.content.click(function() {
            if (Lightbox.config.advanceOnClick) {
                Lightbox.next();
            }
        });
    ...
    bindEvents: function(){
        if(Lightbox.config.bindKeyboard){
    ...
    next: function() {
        ...
        if (totalContent > 1) {
            ...
            } else if (Lightbox.contentType === "content") {
                Lightbox.setContent(Lightbox.contents[Lightbox.currentIndex])
            }
        }
    },
